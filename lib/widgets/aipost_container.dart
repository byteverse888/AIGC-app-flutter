...
class AIPostContainer extends StatelessWidget {
...
void _onCreateAIComment(ParseObject post) {
  //print("评论");
  AppNavigator.createAIComment(post: post);
}

void _onLike(ParseObject post) {
  DatabaseService.likePost(post['objectId']!);
}

void _onReport(String userId, ParseObject post) {
  DatabaseService.reportPost(userId, post['objectId']!);
}

void _onDelete(ParseObject post) async {
  //IMUtil;
  ...
  if (confirm == true) {
    try {
      await DatabaseService.deletePost(post['objectId']!);
    } catch (e) {
